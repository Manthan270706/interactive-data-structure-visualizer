...
import main.models.queues.LinearQueue;

public class LinearQueueVisualizer {
    private VBox layout;
    private HBox controlPanel;
    private Pane canvas;
    private LinearQueue queue;
    private static final int CAPACITY = 8;

    public LinearQueueVisualizer() {
        layout = new VBox(20);
        layout.setPadding(new Insets(20));

        queue = new LinearQueue(CAPACITY);
        initializeUI();
    }

    private void initializeUI() {
        // Section title
        Label title = new Label("Linear Queue");
    ...
    private HBox createControlPanel() {
            ...
            if (!val.isEmpty()) {
                try {
                    if (queue.enqueue(val)) {
                        ...
                        showAlert("Queue Full", "Cannot enqueue. Queue is full!");
                    }
                } catch (Exception ex) {
                    showAlert("Error", "An error occurred while enqueueing: " + ex.getMessage());
                }
            } else {
                showAlert("Input Error", "Please enter a value to enqueue.");
            }
        });
        ...
        dequeueBtn.setOnAction(e -> {
            try {
                ...
                if (dequeuedValue != null) {
                    ...
                    showAlert("Queue Empty", "Cannot dequeue. Queue is empty!");
                }
            } catch (Exception ex) {
                showAlert("Error", "An error occurred while dequeueing: " + ex.getMessage());
            }
        });
        ...
        clearBtn.setOnAction(e -> {
            try {
                ...
                showAlert("Queue Cleared", "Queue has been cleared successfully.");
            } catch (Exception ex) {
                showAlert("Error", "An error occurred while clearing the queue: " + ex.getMessage());
            }
        });
    ...
    private void drawQueue() {
        ...
        for (int i = 0; i < CAPACITY; i++) {
            ...
            // Fill color based on content - only show occupied slots between front and rear
            if (i >= frontIndex && i <= rearIndex && queueArray[i] != null) {
            ...
            // Value text - only show values for occupied slots
            String displayValue = "empty";
            if (i >= frontIndex && i <= rearIndex && queueArray[i] != null) {
                displayValue = queueArray[i];
            }
            ...
            if (i == frontIndex && queueArray[i] != null) {
                drawFrontPointer(x + nodeWidth/2, startY);
            }
            if (i == rearIndex && queueArray[i] != null) {
