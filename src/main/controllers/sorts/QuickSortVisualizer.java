...
import main.models.sorts.QuickSortAlgorithm;
...
public class QuickSortVisualizer {
    ...
    private QuickSortAlgorithm sorter;
    ...
    private void createUI() {
        ...
        Label header = new Label("Quick Sort");
    ...
    private void prevStep() {
        if (currentStep > 0) {
            currentStep--;
            int[] arr = sorter.getHistory().get(currentStep);
            int[] pair = sorter.getCompares().get(currentStep);
            drawArray(toList(arr), pair[0], pair[1],
                    sorter.getDescriptions().get(currentStep).contains("Comparing") ||
                            sorter.getDescriptions().get(currentStep).contains("Swapping"));
            stepText.setText(sorter.getDescriptions().get(currentStep));
        } else {
    ...
    private void showStep(int index) {
        ...
        drawArray(toList(sorter.getHistory().get(index)), pair[0], pair[1],
                desc.contains("Comparing") || desc.contains("Swapping"));
    }
    ...
    private void drawArray(List<Integer> arr, int compare1, int compare2, boolean highlight) {
        ...
        if (highlight && compare1 != -1 && compare2 != -1) {
